...
export const APP_NAME = process.env.APP_NAME || "Syncfuse";
export const AUTH_TOKEN = `${(process.env.APP_NAME || "syncfuse").toLowerCase()}.auth`;
export const LOCALE_CONFIG = `${(process.env.APP_NAME || "syncfuse").toLowerCase()}.locale`;
export const TIMEZONE_CONFIG = `${(process.env.APP_NAME || "syncfuse").toLowerCase()}.timezone`;
export const DATE_RANGE_CONFIG = `${(process.env.APP_NAME || "syncfuse").toLowerCase()}.date-range`;
export const THEME_CONFIG = `${(process.env.APP_NAME || "syncfuse").toLowerCase()}.theme`;
export const DASHBOARD_CONFIG = `${(process.env.APP_NAME || "syncfuse").toLowerCase()}.dashboard`;
export const LAST_TEAM_CONFIG = `${(process.env.APP_NAME || "syncfuse").toLowerCase()}.last-team`;
export const VERSION_CHECK = `${(process.env.APP_NAME || "syncfuse").toLowerCase()}.version-check`;
export const SHARE_TOKEN_HEADER = `x-${(process.env.APP_NAME || "syncfuse").toLowerCase()}-share-token`;
export const HOMEPAGE_URL = process.env.HOMEPAGE_URL || "https://syncfuse.io";
export const DOCS_URL = process.env.DOCS_URL || "https://syncfuse.io/docs";
export const REPO_URL = process.env.REPO_URL || "https://github.com/syncfuse/syncfuse";
export const UPDATES_URL = process.env.UPDATES_URL || "https://api.syncfuse.io/v1/updates";
export const TELEMETRY_PIXEL = process.env.TELEMETRY_URL || "https://i.syncfuse.io/a.png";
...
export const SESSION_COLUMNS = ["browser", "os", "device", "screen", "language", "country", "city", "region"];
...
export const ROLE_PERMISSIONS = {
  ...
  [ROLES.user]: [PERMISSIONS.websiteCreate, PERMISSIONS.websiteUpdate, PERMISSIONS.websiteDelete, PERMISSIONS.teamCreate],
  [ROLES.viewOnly]: [],
  [ROLES.teamOwner]: [PERMISSIONS.teamUpdate, PERMISSIONS.teamDelete, PERMISSIONS.websiteCreate, PERMISSIONS.websiteUpdate, PERMISSIONS.websiteDelete, PERMISSIONS.websiteTransferToTeam, PERMISSIONS.websiteTransferToUser],
  [ROLES.teamManager]: [PERMISSIONS.teamUpdate, PERMISSIONS.websiteCreate, PERMISSIONS.websiteUpdate, PERMISSIONS.websiteDelete, PERMISSIONS.websiteTransferToTeam],
...
export const CHART_COLORS = ["#2680eb", "#9256d9", "#44b556", "#e68619", "#e34850", "#f7bd12", "#01bad7", "#6734bc", "#89c541", "#ffc301", "#ec1562", "#ffec16"];

export const DOMAIN_REGEX = /^(localhost(:[1-9]\d{0,4})?|((?=[a-z0-9-_]{1,63}\.)(xn--)?[a-z0-9-_]+(-[a-z0-9-_]+)*\.)+(xn--)?[a-z0-9-_]{2,63})$/;
export const SHARE_ID_REGEX = /^[a-zA-Z0-9]{8,50}$/;
export const DATETIME_REGEX = /^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]{3}(Z|\+[0-9]{2}:[0-9]{2})?)?$/;
...
export const SOCIAL_DOMAINS = ["bsky.app", "facebook.com", "fb.com", "ig.com", "instagram.com", "linkedin.", "news.ycombinator.com", "pinterest.", "reddit.", "snapchat.", "t.co", "threads.net", "tiktok.", "twitter.com", "x.com"];

export const SEARCH_DOMAINS = ["baidu.com", "bing.com", "chatgpt.com", "duckduckgo.com", "ecosia.org", "google.", "msn.com", "perplexity.ai", "search.brave.com", "yandex."];

export const SHOPPING_DOMAINS = ["alibaba.com", "aliexpress.com", "amazon.", "bestbuy.com", "ebay.com", "etsy.com", "newegg.com", "target.com", "walmart.com"];

export const EMAIL_DOMAINS = ["gmail.", "hotmail.", "mail.yahoo.", "outlook.", "proton.me", "protonmail."];

export const VIDEO_DOMAINS = ["twitch.", "youtube."];

export const PAID_AD_PARAMS = ["ad_id=", "aid=", "dclid=", "epik=", "fbclid=", "gclid=", "li_fat_id=", "msclkid=", "ob_click_id=", "pc_id=", "rdt_cid=", "scid=", "ttclid=", "twclid=", "utm_medium=cpc", "utm_medium=paid", "utm_medium=paid_social", "utm_source=google"];
